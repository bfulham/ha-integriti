...
def test_door_action_uses_database_object_id() -> None:
    root = ET.fromstring(
        IntegritiClient._build_door_action(
            "5066553875759108", on_assert=3, on_deassert=0, unlock_seconds=10
        )
    )
    ...
    assert _text(root, "OnDeAssert") == "0"
...
def test_area_arm_uses_no_deassert_action() -> None:
    root = ET.fromstring(
        IntegritiClient._build_area_action("4503603922337839", arm=True)
    )
    assert _text(root, "OnAssert") == "1"
    assert _text(root, "OnDeAssert") == "0"
...
def test_door_action_can_use_address_reference() -> None:
    root = ET.fromstring(
        IntegritiClient._build_door_action(
            None, address="D38", on_assert=2, on_deassert=0
        )
    )
    ref = root.find("./Entity/Ref")
    assert ref is not None
